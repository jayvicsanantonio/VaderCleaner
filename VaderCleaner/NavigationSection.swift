...
enum NavigationSection: CaseIterable, Hashable, Identifiable {
    ...
    /// matte relief derived from the section's hero art. The rail renders it
    /// neutral when inactive and multiplies it by `theme.accent` when active.
    /// `nil` for Health Monitor, which ships no bespoke art and falls back to
    /// the SF Symbol `icon`. Generated by `Scripts/generate-rail-glyphs.swift`.
